...
case `uname` in
  Linux)
    ...
    g++ -std=c++0x -isystem ${GTEST_DIR}/include -pthread -fprofile-arcs -ftest-coverage server_test.cc request.cc response.cc echo_handler.cc static_handler.cc bad_request_handler.cc session.cc server.cc ${GTEST_DIR}/src/gtest_main.cc libgtest.a -o server_test -lboost_system
    ./server_test && gcov -r server.cc
    g++ -std=c++0x -isystem ${GTEST_DIR}/include -pthread -fprofile-arcs -ftest-coverage session_test.cc request.cc response.cc echo_handler.cc static_handler.cc bad_request_handler.cc session.cc ${GTEST_DIR}/src/gtest_main.cc libgtest.a -o session_test -lboost_system
